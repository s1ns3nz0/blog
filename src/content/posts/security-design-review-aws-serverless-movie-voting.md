...
The starting point is AWS's `aws-serverless-crud-sample` repository, used here as a movie-rating service. What follows is what the plugin can establish from the code, what it has to ask the service owner instead, and why each conclusion holds up the way it does.
...
The service stores this information to publish it. An unauthorized reader learning a movie title or public rating does not disclose an additional secret.
